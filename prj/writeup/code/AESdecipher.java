...
class AESdecipher {
  ...
  private static String aesSBox(int inHex) {
    String result = Integer.toHexString(sBox[inHex]);
    if(result.length() < 2){ 
      result = "0" + result;
    }
    return result;
  ...
  private static String undoSBox(int inHex) {
    String result =  Integer.toHexString(reverseSBox[inHex]);
    if(result.length() < 2){ 
      result = "0" + result;
    }
    return result;
  ...
  private static String aesRCon(int round) {
    String result = Integer.toHexString(rCon[round]);
    if(result.length() < 2){ 
      result = "0" + result;
    }
    return result;
  ...
  private static String[][] AESStateXOR(String[][] sHex, String[][] keyHex) {
    ...
    for (int i = 0; i < 4; i++) {
      for (int j = 0; j < 4; j++) {
        outStateHex[j][i] = XOR(sHex[i][j], keyHex[i][j]);
      }
    }
  ...
  public static String[][] AESInvNibbleSub(String[][] inStateHex) {
    ...
    for (int i = 0; i < 4; i++) {
      for (int j = 0; j < 4; j++) {
        inStateHex[i][j] = undoSBox(Integer.parseInt(inStateHex[i][j], 16));
      }
    }
    return inStateHex;
  }
  ...
  public static String[][] AESInvShiftRow(String[][] inStateHex) {
    ...
    for (int index = 0; index < 4; index++) {
      ...
      shiftedHex[1][index] = inStateHex[1][(index + 3) % 4];
      shiftedHex[2][index] = inStateHex[2][(index + 2) % 4];
      shiftedHex[3][index] = inStateHex[3][(index + 1) % 4];
    }
    return shiftedHex;
  }
  ...
  public static String[][] AESInvMixColumn(String[][] inStateHex) {
    ...
    for (int c = 0; c < 4; c++) {
      inStateHex[0][c] = Integer.toHexString(
              galois(0x0E, M[0][c]) ^ galois(0x0B, M[1][c]) ^ galois(0x0D, M[2][c]) ^ galois(0x09, M[3][c])).toUpperCase();
      inStateHex[1][c] = Integer.toHexString(
              galois(0x09, M[0][c]) ^ galois(0x0E, M[1][c]) ^ galois(0x0B, M[2][c]) ^ galois(0x0D, M[3][c])).toUpperCase();
      inStateHex[2][c] = Integer.toHexString(
              galois(0x0D, M[0][c]) ^ galois(0x09, M[1][c]) ^ galois(0x0E, M[2][c]) ^ galois(0x0B, M[3][c])).toUpperCase();
      inStateHex[3][c] = Integer.toHexString(
              galois(0x0B, M[0][c]) ^ galois(0x0D, M[1][c]) ^ galois(0x09, M[2][c]) ^ galois(0x0E, M[3][c])).toUpperCase();
    }
  ...
  static String AESdecrypt(String pTextHex, String kHex) {
    ...
    String[][] keyHex = generateMatrix(secureKeys[10], 4, 4);
    ...
    for (int key = 9; key > 0; key--) {
      ...
      outStateHex = AESInvMixColumn(outStateHex);
      outStateHex = AESInvShiftRow(outStateHex);
      outStateHex = AESInvNibbleSub(outStateHex);
    }

    keyHex = generateMatrix(secureKeys[0], 4, 4);
    outStateHex = AESStateXOR(outStateHex, keyHex);
